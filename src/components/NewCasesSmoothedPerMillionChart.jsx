...
import { ParseNewCasesSmoothedPerMillion } from '../utils/parseData';
...
const NewCasesSmoothedPerMillionChart = forwardRef(({ selectedCountry3 }, ref) => {
  ...
  useEffect(() => {
    const { pakistanCases, saudiCases } = ParseNewCasesSmoothedPerMillion();
  
    let datasets = [];
    if (selectedCountry3 === 'Pakistan' || selectedCountry3 === 'Both') {
      datasets.push({
        label: 'Pakistan',
    ...
    if (selectedCountry3 === 'Saudi Arabia' || selectedCountry3 === 'Both') {
      datasets.push({
        label: 'Saudi Arabia',
  ...
  }, [selectedCountry3]); // Add selectedCountry as a dependency
...
export default NewCasesSmoothedPerMillionChart;
